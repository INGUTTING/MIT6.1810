...
static char* syscall_name[] = {
    ...
    "mknod", "unlink", "link",   "mkdir", "close", "interpose"};
...
void syscall(void) {
  ...
  if (p->mask & (1 << num)) {
    ...
    if (num == SYS_open || num == SYS_exec) {
      ...
      if (argstr(0, currentPath, sizeof(currentPath)) < 0) {
        p->trapframe->a0 = -1;
        return;
      }
      char allowedPath[MAXPATH];
      acquire(&p->lock);
      safestrcpy(allowedPath, p->allowPath, sizeof(allowedPath));
      release(&p->lock);

      if (strncmp(currentPath, allowedPath, sizeof(currentPath)) != 0) {
        //printf("%d: cannot %s %s\n", p->pid, syscall_name[num], currentPath);
        p->trapframe->a0 = -1;
        return;
      }
      //
...
// if(num > 0 && num < NELEM(syscalls) && syscalls[num]){
//   // to check whether the syscall is not allowed
//   if(p->mask & (1 << num)){
//     // check whether the path is unbounded
//     if(num == SYS_open || num == SYS_exec){
//       // judge the path is matched or not
//       // but not all syscall have path
//       char currentPath[MAXPATH] = {0};
//       if(argstr(0,currentPath,sizeof(currentPath)) < 0){
//         printf("%d:cannot %s %s\n",p->pid,syscall_name[num],currentPath);
//         p->trapframe->a0 = -1;
//         return;
//       }
//       char allowedPath[MAXPATH] = {0};
//       acquire(&p->lock);
//       safestrcpy(allowedPath,p->allowPath,sizeof(allowedPath));
//       release(&p->lock);
//       if(strncmp(currentPath, allowedPath, MAXPATH) != 0){
//         printf("%d: cannot %s %s\n", p->pid, syscall_name[num], currentPath);
//         p->trapframe->a0 = -1;
//         return;
//       }
...
//     return;
//   }
//   // if the mask is not matched ,then skip that action
//   p->trapframe->a0 = syscalls[num]();
// }
// else{
//   printf("%d %s: unknown syscall %d\n",p->pid,syscall_name[num],num);
